...
/**
 * SHA-1 is provided only for compatibility with legacy protocols and checksums.
 * Do not use it as a recommended secure encryption, password, or signature primitive.
 */
object Sha1Utils {
    fun digest(bytes: ByteArray): ByteArray = digestBytes("SHA-1", bytes)
    ...
    fun digest(file: File): ByteArray = digestFile("SHA-1", file)
